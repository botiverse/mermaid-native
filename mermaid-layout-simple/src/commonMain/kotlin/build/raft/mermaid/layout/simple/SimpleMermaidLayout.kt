    ...
    private fun layoutClass(
        ...
        val sizes = diagram.classes.associate { klass ->
            val lines = listOf(klass.label) + klass.members.map { member ->
                val prefix = when (member.visibility) {
                    build.raft.mermaid.core.ClassVisibility.PUBLIC -> "+"
                    build.raft.mermaid.core.ClassVisibility.PRIVATE -> "-"
                    build.raft.mermaid.core.ClassVisibility.PROTECTED -> "#"
                    build.raft.mermaid.core.ClassVisibility.PACKAGE -> "~"
                }
                "$prefix${member.signature}"
            }
            klass.id to SceneSize(
                width = max(120.0, lines.maxOf { textMeasurer.measure(it, style).width } + 24.0),
                height = max(48.0, lines.size * 22.0 + 16.0),
            )
        }
        ...
        diagram.relationships.forEach { relation ->
            ...
            val (from, to) = when (relation.kind) {
                build.raft.mermaid.core.ClassRelationshipKind.INHERITANCE -> targetTop to sourceBottom
                build.raft.mermaid.core.ClassRelationshipKind.ASSOCIATION -> sourceBottom to targetTop
            }
